...
* Every ExecPlan that can affect frontend presentation must define the before-and-after screenshot and video evidence it will capture, the viewports or devices it will cover, the `plans/<name-of-plan>/screenshots/` and `plans/<name-of-plan>/recordings/` artifact paths it will use, and how the final comparison will be reviewed for regressions, quality-of-life fixes, and the professional UI/UX pass required by `DESIGN.md`.
