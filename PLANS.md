...
* Every ExecPlan for substantial UI additions or refactors must state whether `.agents/skills/ui-mockups/SKILL.md` applies. If it applies, the plan must name the `plans/<name-of-plan>/mockups/` artifacts, the reference sample if one is used, and the selected mockup direction before implementation starts.
...
For substantial UI additions or refactors, add a pre-implementation mockup step before code edits when design direction is not already approved. Use `.agents/skills/ui-mockups/SKILL.md`, create a feature prompt, run `npm run stitch:mockups` when Stitch credentials are available, and save options under `plans/<name-of-plan>/mockups/`. The plan should say whether the user supplied a local reference image or Stitch export bundle, whether the sample is copied into `plans/<name-of-plan>/mockups/reference/`, and which option the user selected. If no Stitch credentials are available, record that live mockup generation was skipped instead of fabricating artifacts.
