...
When planning substantial UI additions, UI refactors, major responsive or layout changes, or any frontend presentation work where the user should choose among visual directions, use the repo-local `ui-mockups` skill at `.agents/skills/ui-mockups/SKILL.md` before implementation begins. Save generated mockup artifacts under `plans/<name-of-plan>/mockups/`, where `<name-of-plan>` is the ExecPlan filename without `.md`; the tooling writes `index.html` in that folder and prints a clickable `file://` preview link when generation completes. If the user supplies a local reference image or Google Stitch export bundle, the default influence is style only: palette, typography, density, spacing, radius, shadows, and component feel, not copied layout or content unless the user explicitly asks for that. Record the chosen or hybrid mockup direction in the ExecPlan `Decision Log`, `Plan of Work`, and `Visual Evidence` sections before editing UI code.
...
* Every ExecPlan that can affect frontend presentation must define the before-and-after screenshot and video evidence it will capture, include explicit before-implementation screenshot, after-implementation screenshot, and screenshot UX review steps in `Progress` and `Concrete Steps`, name the viewports or devices it will cover, name the `plans/<name-of-plan>/screenshots/` and `plans/<name-of-plan>/recordings/` artifact paths it will use, and explain how the final comparison will be reviewed for regressions, quality-of-life fixes, and the professional UI/UX pass required by `DESIGN.md` and `.agents/skills/review-ui-screenshots/SKILL.md`.
* Every ExecPlan for substantial UI additions or refactors must state whether `.agents/skills/ui-mockups/SKILL.md` applies. If it applies, the plan must name the `plans/<name-of-plan>/mockups/` artifacts including the generated `index.html` preview page, the reference sample if one is used, and the selected mockup direction before implementation starts.
...
For any ExecPlan that affects frontend presentation, validation must include visual review. Start the app, use `agent-browser` to capture baseline screenshots and videos before making UI-affecting changes, then capture matching after screenshots and videos once the plan is implemented. These captures must appear as concrete execution steps in the plan: the before step comes before implementation edits, the after step comes after implementation, and a separate screenshot UX review step using `.agents/skills/review-ui-screenshots/SKILL.md` comes before final acceptance. Cover multiple resolutions, including desktop and mobile, and name the exact viewport sizes or device presets in the plan. Save screenshots under `plans/<name-of-plan>/screenshots/` as `before-implementation-1.<extension>`, `before-implementation-2.<extension>`, `after-implementation-1.<extension>`, and so on; use the same numbering to pair comparable before and after views. Compare the before-and-after evidence directly. Then run the professional UI/UX review pass required by `DESIGN.md`, checking the result for clear hierarchy, responsive behavior, accessible interaction states, polished spacing and typography, practical workflow ergonomics, and broken or confusing states. Inspect each screenshot section by section so dense panels, controls, and labels are not missed. The plan must say what regressions were found, what fixes or quality-of-life improvements were made as a result, what professional UX concerns remain out of scope, and why those concerns were deferred. If no runnable visual target exists, the plan must state why screenshots or videos cannot be captured and what alternative proof is sufficient.

For substantial UI additions or refactors, add a pre-implementation mockup step before code edits when design direction is not already approved. Use `.agents/skills/ui-mockups/SKILL.md`, create a feature prompt, run `npm run stitch:mockups` when Stitch credentials are available, and save options under `plans/<name-of-plan>/mockups/`. The plan should say whether the user supplied a local reference image or Stitch export bundle, whether the sample is copied into `plans/<name-of-plan>/mockups/reference/`, that the generated `index.html` preview was inspected from the printed `file://` link, and which option the user selected. If no Stitch credentials are available, record that live mockup generation was skipped instead of fabricating artifacts.
...
## Skeleton of a Good ExecPlan
    ...
    When visual evidence applies, include explicit entries for capturing the before screenshots before implementation, capturing the matching after screenshots after implementation, and reviewing the captured evidence with `.agents/skills/review-ui-screenshots/SKILL.md` during the UI/UX review.
